...
This guide focuses on deploying the generated Node server. For the message-driven agent API model, including direct HTTP/WebSocket `/agents/:name/:id` delivery and authored channel applications beneath `/channels/:channel/*`, see [Message-Driven Agents](https://github.com/withastro/flue/blob/main/docs/message-driven-agents.md).
